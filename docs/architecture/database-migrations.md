...
- **Libraries do not create their own tables.** The LangGraph checkpoint tables
  are declared in `titan.storage.checkpoints` and created by a revision, which
  also marks the saver's own migrations as applied, so
  `AsyncPostgresSaver.setup()` is never needed. A test compares the tables with
  what `setup()` would create and fails when a LangGraph upgrade changes them;
